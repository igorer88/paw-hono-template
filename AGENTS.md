...
| `src/services/`             | Cloud-agnostic business logic + Zod schemas. No Hono imports, no HTTP status codes or envelopes. Receive `Bindings` as an explicit parameter (never import wrangler runtime types). Return `Result<T>` from `@/types`. |
| `src/shared/`               | Pure functions and types. No Hono imports. No side effects. `validateInput` (`validate.ts`) lives here; `Result<T>` lives in `src/types.ts`.                                                                           |
