...
Releases are configured **per project** — the template ships the semantic-release pipeline but no bot or host-specific branch-protection config, so the same setup works on any Git host (GitHub, GitLab, etc.). If the project enforces PR-only protection on the release branch (e.g., a GitHub ruleset or a GitLab protected branch), the release commit-back push (`CHANGELOG.md`/version bump) must be authorized by a bot or service token for that host; otherwise the first release fails at the push step. Projects that prefer not to configure a bot can disable the commit-back so versions and changelog live only in git tags and releases.
