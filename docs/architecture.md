...
- Security gates in CI: `codeql.yml` (CodeQL SAST with the `security-extended` query suite) and `dependency-review.yml` (fails PRs introducing high-severity vulnerabilities or disallowed licenses). Both are gated on `github.repository_visibility == 'public'` — skipped on private repos (code scanning and dependency review require GitHub Advanced Security there) and auto-enabled if the template is made public. Secret scanning runs on every push/PR via `secret-scan.yml` (gitleaks, full history) and on staged changes through a pre-commit hook — GitHub's native secret scanning is not available on user-owned private repos on the free plan, so gitleaks covers it.
